...
class GuessLetter(tk.Frame):
    ...
    def __init__(self, parent: Union[tk.Tk, tk.Frame], row: int, col: int, settings: dict) -> None:
        ...
        self.grid(row=row, column=col, padx=settings['ui']['guesses']
                  ['letter_padding'], pady=settings['ui']['guesses']['letter_padding'])

        self.grid_propagate(False)

        f = font.Font(family=settings['ui']['font_family'])

        self.label = tk.Label(self, bg=settings['ui']['guesses']['initial_bg_color'], fg=settings['ui']
                              ['guesses']['initial_text_color'], font=(f, settings['ui']['guesses']['letter_font_size']))
...
class GuessesFrame(tk.Frame):
    ...
    def __init__(self, parent: Union[tk.Tk, tk.Frame], settings: dict) -> None:
        ...
        for i in range(1, settings['num_guesses']+1):
            guesses = []
            for j in range(1, settings['word_size']+1):
                guesses.append(GuessLetter(self, i, j, settings))
            self.guess_letters.append(guesses)
    ...
    def set_letter(self, letter: str, guess_num: int, letter_index: int) -> None:
        ...
        assert guess_num <= self.settings['num_guesses'] and guess_num >= 0
        assert letter_index <= self.settings['word_size'] and letter_index >= 0
    ...
    def show_guess_result(self, guess_num: int, results: list[LetterState]) -> None:
        ...
        """
        for ltr in range(len(self.guess_letters[guess_num])):
            self.update()
            time.sleep(self.settings['ui']['guesses']['process_wait_time'])
            self.guess_letters[guess_num][ltr].set_status(results[ltr])
...
class KeyboardFrame(tk.Frame):
    ...
    def set_key_handler(self, key: str, handler: Callable[[], None]) -> None:
        ...
        handler: Callable[[], None]) The handler function to call
        when the key is pressed.
        """
        assert key.isalpha() or key in ['BACK', 'ENTER']
